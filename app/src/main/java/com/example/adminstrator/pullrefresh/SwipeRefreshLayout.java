...
public class SwipeRefreshLayout extends ViewGroup {
    ...
    public static final int PULL_START = 0;
    public static final int PULL_CAN_REFRESH = 1;
    public static final int PULL_REFRESHING = 2;
    public static final int PULL_REFRESH_COMPLETE = 3;

    private RefreshProgressBar mProgressBar; //the thing that shows progress is going
    private RefreshProgressBar mProgressBarBottom;
    private View mTarget; //the content that gets pulled down
    private int mOriginalOffsetTop;
//    private OnRefreshListener mRefreshListener;
//    private OnLoadListener mLoadListener;
    private SwipeOnRefreshListener mRefreshListener;
    private int mFrom;
    ...
    private static int PULL_DOWN = 0;
    ...
    private int mPullMaxDistance = 0;

    private int mTopPreNotifyDistance = 0;
    private int mBottomPreNotifyDistance = 0;
...
//            }
    ...
    //移动到触发上拉下拉能刷新的位置，显示正在刷新的动画，调用刷新完成，再恢复到原始位置
    private final Animation mAnimationToTriggerPosition = new Animation(){
    	@Override
        public void applyTransformation(float interpolatedTime, Transformation t){
    		int targetTop = 0;
    ...
    public SwipeRefreshLayout(Context context, AttributeSet attrs) {
        ...
        mProgressBar.setTipText("下拉刷新", "松手刷新", "正在刷新");
        mProgressBarBottom = new RefreshProgressBar(getContext(), 1);
        mProgressBarBottom.setTipText("上拉加载", "松手加载", "正在加载");
    ...
    private void animationOffsetToTriggerRefresPosition(int from){
    	 mFrom = from;
    	 mAnimationToTriggerPosition.reset();
    	 mAnimationToTriggerPosition.setDuration(mMediumAnimationDuration);
    	// mAnimationToTriggerPosition.setAnimationListener(listener);
    	 mAnimationToTriggerPosition.setInterpolator(mDecelerateInterpolator);
         mTarget.startAnimation(mAnimationToTriggerPosition);
    }
     ...
     * gesture.
     */
    public void setOnRefreshListener(SwipeOnRefreshListener listener) {
...
//                final float yDiff = y - mInitialMotionY;
    ...
    boolean canPullRefresh = false;
    boolean canPullLoad = false;
...
//                final float yDiff = y - mInitialMotionY;
                ...
                if (mIsBeingDragged) {
                    ...
                    if (yDiff > mDistanceToTriggerSync) {
                    	...
                    	//	startRefresh();
                    		canPullRefresh = true;
                    		mProgressBar.setState(PULL_DOWN, PULL_CAN_REFRESH);
                    	}
                    }
                    	...
                    	{
                    		mLastDirection = Mode.PULL_FROM_END;
                    		mProgressBarBottom.setState(PULL_UP, PULL_CAN_REFRESH);
                    		canPullLoad = true;
                    		//startLoad();
                    	}
					}else {
						mProgressBar.setState(PULL_DOWN, PULL_START);
						mProgressBarBottom.setState(PULL_UP, PULL_START);
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                mLastDirection = Mode.DISABLED;
                final int index = MotionEventCompat.getActionIndex(ev);
                float upY = MotionEventCompat.getY(ev, index);
                if(canPullRefresh){
                	//animationOffsetToTriggerRefresPosition(mCurrentTargetOffsetTop + getPaddingTop());
                	setTargetOffsetTopAndBottom((int)(mDistanceToTriggerSync-mCurrentTargetOffsetTop-getPaddingTop()));
                	startRefresh();
                }else if(canPullLoad){
                	setTargetOffsetTopAndBottom((int)(mStartPoint-upY - mDistanceToTriggerSync));
                	startLoad();
                }else{                	
                	updatePositionTimeout1();
                }
                return false;
        }
    ...
    public void refreshComplete(){
    	updatePositionTimeout1();
    	if(mRefreshing)
    		mProgressBar.setState(PULL_DOWN, PULL_REFRESH_COMPLETE);
    	if(mLoading)
    		mProgressBar.setState(PULL_UP, PULL_REFRESH_COMPLETE);
    	mRefreshing = false;
    	mLoading = false;
    ...
    /*
        @params direction 0 scroll donw, 1 scroll up
        @params distance the distance of top or bottom
     */
    public void setPreviousNotifyDistance(int direction, int distance){
        if(0==direction && distance>0){
            mTopPreNotifyDistance = distance;
            setTagetScrollListener();
        }else if(1==direction && distance>0){
            mBottomPreNotifyDistance = distance;
            setTagetScrollListener();
        }
    }

    private void setTagetScrollListener(){
        ensureTarget();
        if(mTarget instanceof AbsListView){
            ((AbsListView)mTarget).setOnScrollListener(new AbsListView.OnScrollListener() {
                private boolean upNotifyed = false;
                private boolean downNotifyed = true;
                ...
                public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
                    if (lastVisibleItem > 0 && mBottomPreNotifyDistance > 0 && (mMode == Mode.BOTH || mMode == Mode.PULL_FROM_END) && lastVisibleItem <= firstVisibleItem) {
                        //上拉提醒
                        if (totalItemCount - firstVisibleItem - visibleItemCount <= mBottomPreNotifyDistance && (!upNotifyed)) {
                            upNotifyed = true;
                            mRefreshListener.onRefresh(2, false);
                        }
                        if (totalItemCount - firstVisibleItem - visibleItemCount > mBottomPreNotifyDistance) {
                            upNotifyed = false;
                        }
                    }

                    if (lastVisibleItem > 0 && mTopPreNotifyDistance > 0 && (mMode == Mode.BOTH || mMode == Mode.PULL_FROM_START) && lastVisibleItem >= firstVisibleItem) {
                        //下拉提醒
                        if (firstVisibleItem <= mTopPreNotifyDistance && (!downNotifyed)) {
                            downNotifyed = true;
                            mRefreshListener.onRefresh(1, false);
                        }

                        if (firstVisibleItem > mTopPreNotifyDistance) {
                            downNotifyed = false;
                        }
                    }
    ...
    private void startRefresh() {
    	if (!mLoading && !mRefreshing)
		{
           ...
           // mReturnToStartPosition.run();
            setRefreshing(true);
            mProgressBar.setState(PULL_DOWN, PULL_REFRESHING);
            if(null != mRefreshListener) {
                mRefreshListener.onRefresh(1, true);
            }
		}
    }
    ...
    private void startLoad() {
    	if (!mLoading && !mRefreshing)
		{
           ...
           // mReturnToStartPosition.run();
            setLoading(true);
            mProgressBarBottom.setState(PULL_UP, PULL_REFRESHING);
            if(null != mRefreshListener) {
                mRefreshListener.onRefresh(2, true);
            }
		}
    }
    ...
    private void updateContentOffsetTop(int targetTop) {
        final int currentTop = mTarget.getTop();
        if (Math.abs(targetTop) > mPullMaxDistance) {
            if(targetTop < 0){
            	targetTop = -mPullMaxDistance;
            }else
            	targetTop = mPullMaxDistance;
...
//        }
    ...
    private void setTargetOffsetTopAndBottom(int offset) {
    	mProgressBar.offsetTopAndBottom(offset);
        mTarget.offsetTopAndBottom(offset);
        mCurrentTargetOffsetTop = mTarget.getTop();
        mProgressBarBottom.offsetTopAndBottom(offset);
    }
    ...
    private void updatePositionTimeout1() {
        removeCallbacks(mCancel);
        post(mCancel);
    }
